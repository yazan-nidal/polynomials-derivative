...
}}//finsh Arrangement for print palnomyal in arrangement and drivets and  integration
}//end fun0
void fun1(char x[100],char x1[100],char x2[100],int &b,int&p,double cpoint[100],int &cpcont){
	...
	int w1=0;
	cpcont=0;
	for(double fg=-10;fg<=10;fg++){
		for(double rt=fg;rt<=(fg+0.99);rt=rt+0.1){ mn=0;
		...
		su=((double)p*(double)b)*pow((double)rt,(double)w);
		if(x[w1]=='-'){mn=(double)mn-(double)su;}
		else{if(x[w1]=='+'){mn=(double)mn+(double)su;}
		else{mn=su;}}}}
		if(mn==0){cpoint[cpcont]=rt;
		cpcont++;}}
...
void fun2(int &cpcont,double cpoint[100],char x[100],char x1[100],char x2[100],int &b,int&p){
	...
	cout<<"\n the cirtical point  ( x , f(x)) : \n\n";
	for(int si1=0;si1<=(cpcont1-1);si1++){cout<<" ( "<<cpoint[si1]<<" , "<<mnp[si1]<<" ) ";}
	cout<<endl;
...
void print2(int x3[100],int x4[100],int j){for(int b=0;b<j;b++){
	if(x4[b]==0) {cout<<"0";
	if(x4[b+1]>0)cout<<"+";
	else {if(x4[b+1]<0)cout<<"-";}}
	else {
	...
	case 1:
		...
		if(x4[b+1]>0)cout<<"+";
			...
			break ; }
	default :{if(b==0){if((x4[b]*x3[b]*(x3[b]-1))==1){cout<<"x^"<<w;}else{if((x4[b]*x3[b]*(x3[b]-1))==(-1)){cout<<"-x^"<<w;}else cout<<x4[b]*x3[b]*(x3[b]-1)<<"x^"<<w;}}
			 else {if(x4[b]<0){x4[b]=x4[b]*(-1);}
			 if((x4[b]*x3[b]*(x3[b]-1))==1){cout<<"x^"<<w;}else{cout<<x4[b]*x3[b]*(x3[b]-1)<<"x^"<<w;}}
			 if(x4[b+1]>0)cout<<"+";
	else {if(x4[b+1]<0)cout<<"-";}}
	}}}}// print2 to cout drivet 2 in arrangemnt
void print3(int x3[100],int x4[100],int j){for(int b=0;b<j;b++){
	...
	}}}cout<<"+c\n";}// print3 to cout Integrationin arrangemnt
void main (){
char x[100];
char x1[100];
int i=0;
...
cout<<"\n\\-------------------------------------------------------------------\n";
	...
	cout<<"	5. Critical points and Max value \n";
	cout<<"	6. End of program  \n";
cout<<"\\-------------------------------------------------------------------\n\n";
	q=0;
	cout<<"\n";
	while(q!=6){
	...
	break ;}
		...
		break ; }
			...
			print3(x3,x4,j);
			   break ;}	 
			   case 5:{fun0(x,x3,x4,x5,j);
					   fun1(x,x1,x2,b,p,cpoint,cpcont);
...
cout<<"\n\n\\-------------------------------------------------------------------\n\n";
	...
	cin>>q;
	cout<<"\n";}//end while
	cout<<"\n****Thank you for using my genius program!****\n\n";
	cout <<"\n ----------------end of program---------------- \n\n ";
